...
namespace edm {
  ...
  protected:
    ...
    virtual void preActionBeforeRunEventAsyncBase(WaitingTask* iTask,
    ...
    const std::vector<std::string>& processesWithSelectedMergeableRunProducts() {
      return processesWithSelectedMergeableRunProducts_;
    }

  private:
    struct BranchIDhash {
      BranchIDhash() {}
      size_t operator()(const BranchID& b) const { return tbb::tbb_hasher(b.id()); }
    };
    struct ParentageIDhash {
      ParentageIDhash() {}
      size_t operator()(const ParentageID& b) const { return b.smallHash(); }
    };
    ...
    void writeStoredMergeableRunProductMetadata(RootOutputFile* rootOutputFile);
