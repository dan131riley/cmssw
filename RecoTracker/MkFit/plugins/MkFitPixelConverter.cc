...
class MkFitPixelConverter : public edm::global::EDProducer<> {
public:
  explicit MkFitPixelConverter(edm::ParameterSet const& iConfig);
  ~MkFitPixelConverter() override = default;
...
MkFitPixelConverter::MkFitPixelConverter(edm::ParameterSet const& iConfig)
...
void MkFitPixelConverter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add("mkFitPixelConverterDefault", desc);
}

void MkFitPixelConverter::produce(edm::StreamID iID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  // Then import hits
...
nullptr_t MkFitPixelConverter::clusterCharge(const SiPixelRecHit& hit, DetId hitId) const { return nullptr; }

bool MkFitPixelConverter::passCCC(float charge) const { return charge > minGoodStripCharge_; }

bool MkFitPixelConverter::passCCC(nullptr_t) const { return true; }

void MkFitPixelConverter::setDetails(mkfit::Hit& mhit, const SiPixelCluster& cluster, int shortId, nullptr_t) const {
  mhit.setupAsPixel(shortId, cluster.sizeX(), cluster.sizeY());
}

void MkFitPixelConverter::setDetails(mkfit::Hit& mhit, const SiStripCluster& cluster, int shortId, float charge) const {
...
DEFINE_FWK_MODULE(MkFitPixelConverter);
